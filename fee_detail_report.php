...
window.onload=function(){
	...
	//alert(page);
	var fee_dtails='<?php echo $_GET['fee_dtails']; ?>';
	var paymode='<?php echo $_GET['paymode']; ?>';
	//alert(fee_dtails)
	if(to!="" && from!=""){
		$("#d_to").val(to);
		$("#d_from").val(from);
		$("#fee_dtails").val(fee_dtails);
		$("#paymode").val(paymode);
	}
	     ...
	     var xhttp = new XMLHttpRequest();
			...
			if(fee_dtails=='' || paymode=='')
			{ 
			//alert("1");
			xhttp.open("GET", "fee_report_ajax.php?from="+from+"&to="+to+"&page="+page, true);
			xhttp.send();
			}
		    if(fee_dtails!='' || paymode!='')
			{
				//alert("2");
			xhttp.open("GET", "fee_particular_report_ajax.php?from="+from+"&to="+to+"&page="+page+"&fee_dtails="+fee_dtails+"&paymode="+paymode, true);
			xhttp.send();
			}
     ...
     <div class="col-md-12">
     	<div class="row">
        	<div class="col-md-4"></div><!-- col-md-4 closed -->
        	 <div class="col-md-4">
                 <h3 class="text-center">Fee Detail Reports</h3>
                 <hr class="style13"/><br />
             </div>
           ...
           <div class="row">
                    ...
                    <div class="col-md-2"><b style="font-size:15px;">Particulars</b></div>
               		 <div class="col-lg-3 col-md-3 col-sm-6 col-xs-6">
                                	<select name="fee_dtails"  id="fee_dtails" class="form-control">
                                    	<option value="">Select</option>
                                    	<option value="adm_form">Adm Form</option>
                                    	<option value="adm_charge">Adm Charge</option>
                                    	<option value="monthly_fee">Monthly Fee</option>
                                    	<option value="trans_fee">Transport Fee</option>
                                        <option value="annual_fee">Annual Dev Fee</option>
                                    	<option value="tie_belt_diary">Tie-Belt-Diary Fee</option>
                                        <option value="book_cover_copies">Book-Cover-Copy Fee</option>
                                        <option value="fine_fee">Fine Fee</option>
                                        <option value="other_fee">Others Fee</option>
                                    </select>
                                </div>
           ...
           <div class="col-md-2"><b>Payment Type</b></div>
           <div class="col-md-3">
        ...
        <br />
          ...
          <?php  //include_once "fee_report_ajax.php";?>
	...
	$("#search_value").val("");
	 //alert(paymode);
	  if(to==""){
		...
		if(fee_dtails=='' || paymode=='')
		{
		xhttp.open("GET", "fee_report_ajax.php?from="+from+"&to="+to, true);
	 	xhttp.send();
		}
	  if(fee_dtails!='' || paymode!='')
		{
		xhttp.open("GET", "fee_particular_report_ajax.php?from="+from+"&to="+to+"&fee_dtails="+fee_dtails+"&paymode="+paymode, true);
	 	xhttp.send();
		}
  }
 </script>

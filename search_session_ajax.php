     ...
     <div class="row">
         ...
         <form action="#" method="" name="export_excel">
			...
			<div class="control-group">
				<div class="controls">
					<a href="export_session_wise.php?session=<?php echo $session;  ?>" id="export" name="export" class="btn btn-primary button-loading" data-loading-text="Loading...">Export MySQL Data to CSV/Excel File</a>
				</div>
			</div>
		</form>
 ...
 <?php
  $count=12;
	...
	$offset=$page*$count;
 	...
 	$sql="SELECT * FROM new_registration WHERE session='$session' limit $offset, $count";
	$sql1="select count(admission_no) as count from `new_registration` WHERE session='$session'";
	...
	while($result=mysqli_fetch_array($rs)){
            ...
            <tr>
                ...
                <td style="text-align:center"><?php echo date('d-m-Y',strtotime($result['admission_date'])); ?></td>
                <td style="text-align:center"><?php echo $result['roll_no']; ?></td>
                <td style="text-align:center"><?php echo $result['section']; ?></td>
                <td style="text-align:center"><?php echo $result['session']; ?></td>
                <td style="text-align:center"><?php echo $result['class']; ?></td>
                <td style="text-align:center"><?php echo date('d-m-Y',strtotime($result['dob'])); ?></td>
                ...
                <?php if($role=='admin'){      ?> 
				  ...
				  <a href='delete_student.php?student_id=<?php echo $result['admission_no']; ?>'><span class="fa fa-trash"></span></a>
                               ...
                               <?php 
							     if($page>1){
									   ?>
										...
										<li><a href ="search_session_wise.php?session=<?php echo $session;?>&page=<?php echo $prev;?>">prev</a></li>
								...
								for($i=0;$i<ceil($pages);$i++){
									if($i==$page-1){
								   ?>
									   <li class="active"><a href="search_session_wise.php?session=<?php echo $session;?>&page=<?php echo $i ?>"><?php echo $i+1;?> </a></li>
									 <?php   
									 }
									 else{
								   ?>
									   <li><a href="search_session_wise.php?session=<?php echo $session;?>&page=<?php echo $i ?>"><?php echo $i+1;?> </a></li>
									 <?php   
									 }
								   }//for closed			 
							       if($page<ceil($pages)){
											?>	
											<li><a href="search_session_wise.php?session=<?php echo $session;?>&page=<?php echo $page;?>">next</a></li>
										   <?php 
